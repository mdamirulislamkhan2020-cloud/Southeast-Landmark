...
function rowToLead(r: LeadRow): CrmLead {
  return {
    ...
    notes: (r.notes ?? []) as CrmLead["notes"],
    tasks: (r.tasks ?? []) as CrmLead["tasks"],
    attachments: (r.attachments ?? []) as CrmLead["attachments"],
    communications: (r.communications ?? []) as CrmLead["communications"],
    timeline: (r.timeline ?? []) as CrmLead["timeline"],
